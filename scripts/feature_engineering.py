...
def add_features(df, dataset_type, old_streaks=None):
    if dataset_type == 'combined':
        ...
        # Calculate streaks for the combined data
        df = calculate_streaks(df)
        
        # Debug: Print streaks from the combined data
        print("\nCombined matches with streaks calculated:")
        print(df[['Home', 'Home_Streak_Wins', 'Away', 'Away_Streak_Losses']].head(20))
        
    else:
        raise ValueError("Invalid dataset type provided. Use 'combined'.")
...
def apply_feature_engineering():
    ...
    # Add features to combined matches and calculate streaks
    combined_matches = add_features(combined_matches, 'combined')

    # Separate new matches from combined data based on original index
    new_matches = combined_matches[combined_matches['Date'] >= '2023-08-01']
